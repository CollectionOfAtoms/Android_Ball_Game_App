...
public abstract class GameThread extends Thread {
	...
	//Helper function to determine collision distance between two round bitmap objects
    public float calcCollisionDistance( Bitmap obj1, Bitmap obj2){
        float collisionDistance;
        //use pythagorean theorem to determine distance between center of two circles
        collisionDistance = (float)Math.sqrt( Math.pow(obj1.getWidth() / 2, 2) + Math.pow(obj2.getWidth() / 2 , 2) );
        return collisionDistance;
    }

    //Helper function to calculate magnitude of vector given components
    public float calcMagnitude( float xComp, float yComp){
        float magnitude = (float)Math.sqrt( Math.pow(xComp, 2) + Math.pow(yComp, 2) );
        return magnitude;
    }
